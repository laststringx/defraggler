...
// points fetch
$result=$mysqli->query("SELECT points_6,total FROM `result` WHERE team_name='$team_name'") or die ("databse fetch problem");
...
{
localStorage.total6=Number(localStorage.total6)+1;
  if (localStorage.clickcount6>0)  //sessionStorage say tab close karnay prr reset hojayga
    {
    localStorage.clickcount6=Number(localStorage.clickcount6)-1;
	 document.getElementById("result").innerHTML="optimal number of initial moves(for full credits): "+localStorage.clickcount6;
    }
   else
    {
	if(localStorage.extra6>0)
	{
	 localStorage.extra6=Number(localStorage.extra6)-1;
document.getElementById("result").innerHTML="extra number of moves(less credits) "+localStorage.extra6;
}
if(localStorage.extra6<=0)
 {
...
//buy localstorage
localStorage.extra6=Number(localStorage.extra6)+v;
...
document.getElementById('numberofcredits1').innerHTML="You have "+vall[1]+" credits";
	...
	var optimal = localStorage.getItem('clickcount6');
	document.getElementById("optimal").value=optimal;
	var extra = localStorage.getItem('extra6');
...
$("#credit").hide();
	...
	var total = localStorage.getItem('total6');
	...
	document.getElementById("credit").style.display='none';
		...
		document.getElementById("A").innerHTML='<div id="box"><img src="images/6/A.png"></div>';
		document.getElementById("B").innerHTML='<div id="box"><img src="images/6/B.png"></div>';
		document.getElementById("C").innerHTML='<div id="box"><img src="images/6/C.png"></div>';
		document.getElementById("D").innerHTML='<div id="box"><img src="images/6/D.png"></div>';
		document.getElementById("E").innerHTML='<div id="box"><img src="images/6/E.png"></div>';
		document.getElementById("F").innerHTML='<div id="box"><img src="images/6/F.png"></div>';
		document.getElementById("G").innerHTML='<div id="box"><img src="images/6/G.png"></div>';
		document.getElementById("H").innerHTML='<div id="box"><img src="images/6/H.png"></div>';
		document.getElementById("I").innerHTML='<div id="box"><img src="images/6/I.png"></div>';
	...
	<title>Question.6 || Defraggler</title>
...
<nav class="navbar navbar-inverse">
  <div class="container-fluid">
    <div class="navbar-header">
      ...
      <a class="navbar-brand" href="#">Defraggler || Question-6</a>
    </div>
	  ...
	  <div id="modal1" class="modal fade bs-example-modal-lg" tabindex="-1" role="dialog">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
				...
				<br>Number of optimal moves : 8<br><br>
...
$result=$mysqli->query("SELECT temp_6 FROM `match` WHERE team_name='$team_name'") or die ("databse fetch problem");
while($row=mysqli_fetch_array($result))
{
...
Prateek and Akshay are playing a game. Rules of the game are quite simple.
Initially there are N piles of stones on the table. In each turn,a player
can choose one pile and remove it from the table.Each player want to 
maximize the total number of stones removed by him.Prateek takes the first
turn.Please tell Prateek the maximum number of stones he can remove 
assuming that both players play optimally.

Input: The first line of the input contains an integer T denoting the number of 
test cases.The description of T test cases follows.The first line of each
test case contains a single integer N denoting the number of piles.
The second line contains N space separated integers A1, A2, ..., AN 
denoting the number of stones in each pile.

Output: For each test case, output a single line containg the maximum number 
of stones that Prateek can remove.
