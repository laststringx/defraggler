...
if ($_POST['action'] == 'submit') {
	$result=$mysqli->query("SELECT string_8,output_8 FROM `answer` WHERE id=1") or die ("databse fetch problem");
	while($row=mysqli_fetch_array($result))
	{
		$string=$row['string_8'];
		
		$output1=$row['output_8'];
	...
	if($totalmoves<=0){$points_8=75;}
	else $points_8=75-$totalmoves;
	
	$total+=$points_8;
	$stmt1=$mysqli->prepare("UPDATE result SET points_8=?,total=? WHERE team_name=?");
	$stmt1->bind_param('dds',$points_8,$total,$team_name);
	if($stmt1->execute()){
...
// use session variable and question number and number of moves to update database
	...
	$stmt1=$mysqli->prepare("UPDATE result SET points_8=? WHERE team_name=?");
	$stmt1->bind_param('ds',$points_8,$team_name);
	if($stmt1->execute()){
		echo "rows updated successfully: ".$stmt1->affected_rows;
	}
	...
	$stmt1=$mysqli->prepare("UPDATE `match` SET temp_8=? WHERE team_name =?");
