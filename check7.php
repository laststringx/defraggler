...
if ($_POST['action'] == 'submit') {
	$result=$mysqli->query("SELECT string_7 FROM `answer` WHERE id=1") or die ("databse fetch problem");
	...
	if($totalmoves<=0){$points_7=50;}
	else $points_7=50-$totalmoves;
	
	$total+=$points_7;
	$stmt1=$mysqli->prepare("UPDATE result SET points_7=?,total=? WHERE team_name=?");
	$stmt1->bind_param('dds',$points_7,$total,$team_name);
	if($stmt1->execute()){
...
// use session variable and question number and number of moves to update database
	...
	$stmt1=$mysqli->prepare("UPDATE result SET points_7=? WHERE team_name=?");
	$stmt1->bind_param('ds',$points_7,$team_name);
	if($stmt1->execute()){
		echo "rows updated successfully: ".$stmt1->affected_rows;
	}
	...
	$stmt1=$mysqli->prepare("UPDATE `match` SET temp_7=? WHERE team_name =?");
