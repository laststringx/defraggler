...
if ($_POST['action'] == 'submit') {
	$result=$mysqli->query("SELECT string_3 FROM `answer` WHERE id=1") or die ("databse fetch problem");
	...
	if($totalmoves<=0){$points_3=50;}
	else $points_3=50-$totalmoves;
	
	$total+=$points_3;
	$stmt1=$mysqli->prepare("UPDATE result SET points_3=?,total=? WHERE team_name=?");
	$stmt1->bind_param('dds',$points_3,$total,$team_name);
	if($stmt1->execute()){
...
// use session variable and question number and number of moves to update database
	...
	$stmt1=$mysqli->prepare("UPDATE result SET points_3=? WHERE team_name=?");
	$stmt1->bind_param('ds',$points_3,$team_name);
	if($stmt1->execute()){
		echo "rows updated successfully: ".$stmt1->affected_rows;
	}
	...
	$stmt1=$mysqli->prepare("UPDATE `match` SET temp_3=? WHERE team_name =?");
