...
// points fetch
$result=$mysqli->query("SELECT points_2,total FROM `result` WHERE team_name='$team_name'") or die ("databse fetch problem");
...
{
localStorage.total2=Number(localStorage.total2)+1;
  if (localStorage.clickcount2>0)  //sessionStorage say tab close karnay prr reset hojayga
    {
    localStorage.clickcount2=Number(localStorage.clickcount2)-1;
	 document.getElementById("result").innerHTML="optimal number of initial moves(for full credits): "+localStorage.clickcount2;
    }
   else
    {
	if(localStorage.extra2>0)
	{
	 localStorage.extra2=Number(localStorage.extra2)-1;
document.getElementById("result").innerHTML="extra number of moves(less credits) "+localStorage.extra2;
}
if(localStorage.extra2<=0)
 {
...
//buy localstorage
localStorage.extra2=Number(localStorage.extra2)+v;
...
document.getElementById('numberofcredits1').innerHTML="You have "+vall[1]+" credits";
	...
	var optimal = localStorage.getItem('clickcount2');
	document.getElementById("optimal").value=optimal;
	var extra = localStorage.getItem('extra2');
...
$("#credit").hide();
	...
	document.getElementById("credit").style.display='none';
		...
		document.getElementById("A").innerHTML='<div id="box"><img src="images/2/A.png"></div>';
		document.getElementById("B").innerHTML='<div id="box"><img src="images/2/B.png"></div>';
		document.getElementById("C").innerHTML='<div id="box"><img src="images/2/C.png"></div>';
		document.getElementById("D").innerHTML='<div id="box"><img src="images/2/D.png"></div>';
		document.getElementById("E").innerHTML='<div id="box"><img src="images/2/E.png"></div>';
		document.getElementById("F").innerHTML='<div id="box"><img src="images/2/F.png"></div>';
		document.getElementById("G").innerHTML='<div id="box"><img src="images/2/G.png"></div>';
		document.getElementById("H").innerHTML='<div id="box"><img src="images/2/H.png"></div>';
	...
	<title>Question.2 || Defraggler</title>
...
<nav class="navbar navbar-inverse">
  <div class="container-fluid">
    <div class="navbar-header">
      ...
      <a class="navbar-brand" href="#">Defraggler || Question-2</a>
    </div>
...
$result=$mysqli->query("SELECT temp_2 FROM `match` WHERE team_name='$team_name'") or die ("databse fetch problem");
while($row=mysqli_fetch_array($result))
{
...
Mr. Pranav Malhotra once bought a cow. 
The cow produces it's first she-calf at the age of 2 years 
and after that produces another single she-calf every year. 
After sleeping for years Mr. Pranav Malhotra counted the 
number of total cows to know if any died or not.
He found a pattern and that pattern was fibonacci series.

input: a single number

output: stating if the number is fibonacci or not.

example: 

input: 13

output: Number is in fibonnaci series.
</pre>
</div>
 </div>
</body>
