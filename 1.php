...
// points fetch
$result=$mysqli->query("SELECT points_1,total FROM `result` WHERE team_name='$team_name'") or die ("databse fetch problem");
...
localStorage.total=Number(localStorage.total)+1;
  if (localStorage.clickcount>0)  //sessionStorage say tab close karnay prr reset hojayga
    {
    localStorage.clickcount=Number(localStorage.clickcount)-1;
	 document.getElementById("result").innerHTML="optimal number of initial moves(for full credits): "+localStorage.clickcount;
    }
   else
    {
	if(localStorage.extra>0)
	{
	 localStorage.extra=Number(localStorage.extra)-1;
document.getElementById("result").innerHTML="extra number of moves(less credits) "+localStorage.extra;
}
...
//buy localstorage
localStorage.extra=Number(localStorage.extra)+v;
...
document.getElementById('numberofcredits1').innerHTML="You have "+vall[1]+" credits";
	...
	var optimal = localStorage.getItem('clickcount');
...
$("#credit").hide();
	...
	document.getElementById("credit").style.display='none';
		...
		document.getElementById("A").innerHTML='<div id="box"><img src="images/1/A.png"></div>';
		document.getElementById("B").innerHTML='<div id="box"><img src="images/1/B.png"></div>';
		document.getElementById("C").innerHTML='<div id="box"><img src="images/1/C.png"></div>';
		document.getElementById("D").innerHTML='<div id="box"><img src="images/1/D.png"></div>';
		document.getElementById("E").innerHTML='<div id="box"><img src="images/1/E.png"></div>';
		document.getElementById("F").innerHTML='<div id="box"><img src="images/1/F.png"></div>';
		document.getElementById("G").innerHTML='<div id="box"><img src="images/1/G.png"></div>';
		document.getElementById("H").innerHTML='<div id="box"><img src="images/1/H.png"></div>';
		document.getElementById("I").innerHTML='<div id="box"><img src="images/1/I.png"></div>';
	...
	<title>Question.1 || Defraggler</title>
...
<nav class="navbar navbar-inverse">
  <div class="container-fluid">
    <div class="navbar-header">
      ...
      <a class="navbar-brand" href="#">Defraggler || Question-1</a>
    </div>
...
$result=$mysqli->query("SELECT temp_1 FROM `match` WHERE team_name='$team_name'") or die ("databse fetch problem");
while($row=mysqli_fetch_array($result))
{
...
<div class="row">
	...
	<input type="submit" class="btn btn-primary" name="action" onclick="saveArrangableNodes()" value="skip"/><br>
	<input type="button" id="credit" class="btn btn-primary" data-toggle="modal" data-target="#largeModal" value="Buy Moves"/>
	</div>
...
Once Atul Dhariwal, a topper in mathematics, was playing with
numbers. He wanted to know whether a number is an armstrong 
number or not.
An Armstrong number is a number that is equal to the sum of 
each of the individual digits that make up the number raised 
to the power of the number of digits the number has. 
He wrote a program to find out whether a number is an armstrong
 number or not.

input: a single number

output: resulting whether the number is armstrong or not.
